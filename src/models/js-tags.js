...
class Tags {
  ...
  /**
   * Adds the provided value to the count associated to key, otherwise adds the
   * key-value pair
   *
   * @param {string} key
   * @param {number} val
  ...
  /**
   * Returns a sorted array of objects containing name and count values
   *
   * @param {boolean} order
   * @returns {Array}
   */
   ...
   */
  toString(order = true) {
    return this.sort(order).reduce(
      (acc, { count, name }) =>
        `${acc}${name.padEnd(this._maxLength + 2, ' ')}${count}\n`,
      ''
    )
  }
}
